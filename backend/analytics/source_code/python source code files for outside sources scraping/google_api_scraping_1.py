# -*- coding: utf-8 -*-
"""Google_api_scraping_1.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1WBWY7LvGu7VdrBLUrwoCxm5VLUNu_uRl
"""

import urllib
import bs4
import requests
import webbrowser
import pandas as pd
from selenium import webdriver
from bs4 import BeautifulSoup
import time
from bs4.element import Tag
import path
import os
df = pd.read_csv("CIKtoTicker.csv")
companies = df["Company"].tolist()
companies = [' '.join(x.lower().split()[:-1]) for x in companies]
print(companies)

df2 = pd.DataFrame()
df2["Company"]=[]
df2["Market Size"] =[]
df2["Text"]=[]


company = "apple"


with open("customers_scraped_google.csv",'a+') as file:
    for company in companies:
        text = f'What is the market size of {company}?'
        text = urllib.parse.quote_plus(text)

        url = 'https://google.com/search?q=' + text




        path = r"C:/Users/gitan/Downloads/chromedriver_win32/chromedriver.exe"
        driver = webdriver.Chrome(path)

        driver.get(url)
        time.sleep(3)

        soup = BeautifulSoup(driver.page_source,'lxml')
        result_div = soup.find_all('span', attrs={'class': 'hgKElc'})
        try:
            highlighted = result_div[0]
            bolded = result_div[0].find_all('b')

            df2.append({"Company":company, "Customers":bolded[0].text, "Text":highlighted.text}, ignore_index= True)
            file.write(f"{company}, {bolded[0].text}, {highlighted.text}\n")
            print(bolded[0].text)
        except:
            print(f"Could not find for {company}")

    output_path='marketsize_scraped_google.csv'
    print(df2)
    df2.to_csv(output_path,mode='a', header=not os.path.exists(output_path), index = False)

df2.to_csv(output_path,mode='a', header=not os.path.exists(output_path), index = False)
df

print(df2)

import selenium
import os
from selenium import webdriver
import pandas as pd

path = r"C:/Users/gitan/Downloads/chromedriver_win32/chromedriver.exe"
driver = webdriver.Chrome(path)

    #os.rename(r'C:\Users\Pranav\Downloads\Financial_Report.xlsx', r'C:\Users\Pranav\Downloads\report' + str(i) +'.xlsx')

!pip install path